...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="../" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current">Siswa</a> </div>
    <h1>Siswa</h1>
  </div>
    ...
    <div class="row-fluid">
      <div class="span12">
        ...
        if (isset($_POST['simpan'])) {
          ...
          $nisn = mysqli_real_escape_string($link, $_POST['nisn']);
          $nama = mysqli_real_escape_string($link, $_POST['nama']);
          $jk = mysqli_real_escape_string($link, $_POST['jk']);
          $tmpt_lhr = mysqli_real_escape_string($link, $_POST['tmpt_lhr']);
          $tgl_lahir = mysqli_real_escape_string($link, $_POST['tgl_lahir']);
        $id_kelas = mysqli_real_escape_string($link, $_POST['id_kelas']);

        $log = mysqli_query($link,"select * from siswa where nama='$nama' && nisn='$nisn'");
        $num=mysqli_num_rows($log);
        if ($num == 1){
          echo '<div class="alert alert-error alert-block"> <a class="close" data-dismiss="alert" href="#">×</a>
            <h4 class="alert-heading">Error!</h4>
            Data <b> Siswa </b> yang anda inputkan Sudah Ada! </div>';
        } else {

        $simpan = mysqli_query($link,"insert into siswa(id, nisn, nama, jk, tmpt_lhr, tgl_lahir, id_kelas) value(UUID(),'$nisn','$nama','$jk','$tmpt_lhr','$tgl_lahir','$id_kelas')");
        if($simpan){
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <?php $log = mysqli_query($link,"select * from siswa");
            $num=mysqli_num_rows($log);
            if ($num >= 1){ ?>
            ...
            <table class="table table-bordered data-table">
              ...
              <tbody>
                <?php
                $no = 1;
                $sql = mysqli_query($link,"SELECT siswa.id, siswa.nisn, siswa.nama, siswa.jk, siswa.tmpt_lhr, siswa.tgl_lahir, kelas.kode, kelas.kelas
                from siswa
              ...
              while($tampil=mysqli_fetch_row($sql)){ ?>
                <tr>
                  ...
                  <td><?php echo $tampil[4]; ?>, <?php echo $tampil[5]; ?></td>
                  <td><?php echo $tampil[6]; ?> <?php echo $tampil[7]; ?></td>
  <td>
    <a href=wali_murid.php?id=<?php echo $tampil[0]; ?> class="btn btn-mini btn-primary btn-condensed" title="Pilih Wali Murid"><i class="icon icon-user"></i> <i class="icon icon-arrow-right"></i></a>
  ...
  <!-- end danger with sound -->
          ...
          <?php } else { ?>
            <h1><center>Data Siswa Masih Kosong <br> <a href="input.php" class="btn btn-primary">Tambah</a> </center></h1>
            <?php } ?>
          </div>
        </div>
      </div>
